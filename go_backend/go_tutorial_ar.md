...
### 12.9 التشغيل التلقائي بعد إعادة التشغيل

إذا كان السيرفر على جهاز محلي أو VPS، `scripts/deploy.sh` يثبّت خدمة systemd (`systemd/watt.service`) تضمن تشغيل Docker تلقائيًا بعد reboot أو انقطاع الكهرباء.

ما يفعله السكربت:

1. يفعّل `docker.service` ليبدأ مع النظام.
2. ينسخ `systemd/watt.service` إلى `/etc/systemd/system/`.
3. يشغّل الخدمة ويضمن بدئها مع النظام.

بعد التشغيل الأول، تأكد:

```bash
sudo systemctl status watt.service
```

> **ملاحظة:** تأكد من أن جهاز السيرفر يحصل دائمًا على نفس IP (مثلاً 192.168.1.107) عبر DHCP reservation أو IP ثابت في الراوتر.

### 12.10 وضع التطوير HTTP فقط
