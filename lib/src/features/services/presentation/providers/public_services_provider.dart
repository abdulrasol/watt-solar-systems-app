...
final publicServiceTypesProvider = FutureProvider<List<CompanyType>>((
  ref,
) async {
  return getIt<PublicServicesRepository>().getTypes();
});

class ServicesCompaniesState {
...
class ServicesCompaniesNotifier extends Notifier<ServicesCompaniesState> {
  ...
  ServicesCompaniesState build() {
    ...
    return ServicesCompaniesState(
      type: type,
      // query: PublicCompaniesQuery(cityId: ref.read(authProvider).user?.city?.id, companyTypes: type.code.trim().isEmpty ? const [] : [type.code]),
      // TODO: remove this when we have a way to get the user's city
      query: PublicCompaniesQuery(
        companyTypes: type.code.trim().isEmpty ? const [] : [type.code],
      ),
    );
  }
