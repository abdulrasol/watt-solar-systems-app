...
class StructureDesignCalculator {
  ...
  FrameResult calculate(StructureDesignInput input) {
    ...
    final appliedAzimuth = _resolveAppliedAzimuth(
      idealAzimuth: idealAzimuth,
      preference: input.facingPreference,
    );
    ...
    final usableWidth = math.max(
      0.0,
      input.siteWidthMeters - (input.sideClearanceMeters * 2),
    );
    final usableDepth = math.max(
      0.0,
      input.siteDepthMeters -
          input.frontClearanceMeters -
          input.rearClearanceMeters,
    );

    final projectedRowDepth = _projectedRowDepthMeters(
      input.panelSpec,
      appliedTilt,
    );
    final heightRise = _heightRiseMeters(input.panelSpec, appliedTilt);
    final rowSpacing = _rowSpacingMeters(
      latitude: input.latitude,
      heightRise: heightRise,
    ...
    final rows = _resolveManualCount(
      requested: input.manualRows,
      maximum: autoLayout.rows,
      fallback: autoLayout.rows,
    );
    final columns = _resolveManualCount(
      requested: input.manualColumns,
      maximum: autoLayout.columns,
      fallback: autoLayout.columns,
    );
    ...
    final railLength = columns <= 0 ? 0.0 : rowSlopeLength * 2 * rows;
    final braceLength = math.sqrt(
      math.pow(heightRise, 2) +
          math.pow(projectedRowDepth.clamp(0.0, double.infinity) * 0.65, 2),
    );
    ...
    final rowResults = List<RowFrameResult>.generate(rows, (index) {
      final baseOffset = input.rowMode == RowMode.stepped
          ? rowOffsets[index]
          : 0.0;
    ...
    final minFrontLeg = rowResults.isEmpty
        ? input.frontLegClearanceMeters
        : rowResults.map((row) => row.frontLegHeightMeters).reduce(math.min);
    final maxFrontLeg = rowResults.isEmpty
        ? input.frontLegClearanceMeters
        : rowResults.map((row) => row.frontLegHeightMeters).reduce(math.max);
    final minRearLeg = rowResults.isEmpty
        ? rearLegHeight
        : rowResults.map((row) => row.rearLegHeightMeters).reduce(math.min);
    final maxRearLeg = rowResults.isEmpty
        ? rearLegHeight
        : rowResults.map((row) => row.rearLegHeightMeters).reduce(math.max);
    final isUniformLegDesign =
        rowResults.isEmpty ||
        rowResults.every(
          (row) =>
              (row.frontLegHeightMeters - rowResults.first.frontLegHeightMeters)
                      .abs() <
                  1e-9 &&
              (row.rearLegHeightMeters - rowResults.first.rearLegHeightMeters)
                      .abs() <
                  1e-9,
        );
    final braceCount = rowResults.isEmpty ? 0 : supportStationCount * rows;
    final totalFrontLegLength = rowResults.fold<double>(
      0,
      (sum, row) => sum + (row.frontLegHeightMeters * supportStationCount),
    );
    final totalRearLegLength = rowResults.fold<double>(
      0,
      (sum, row) => sum + (row.rearLegHeightMeters * supportStationCount),
    );
    final totalBraceLength = braceLength * braceCount;
    final totalSteelLength =
        railLength +
        totalFrontLegLength +
        totalRearLegLength +
        totalBraceLength;
    ...
    final bomItems = <BomItem>[
      BomItem(
        ...
        unit: 'm',
        quantity: railLength,
        note: '2 rails x repeated row x row count',
      ),
      BomItem(
        name: 'Front legs',
        unit: 'pcs',
        quantity: frontLegCount.toDouble(),
        note: 'support stations x row count',
      ),
      BomItem(
        name: 'Rear legs',
        unit: 'pcs',
        quantity: rearLegCount.toDouble(),
        note: 'support stations x row count',
      ),
      BomItem(
        name: 'Cross braces',
        unit: 'pcs',
        quantity: braceCount.toDouble(),
        note: '1 brace per support station x row count',
      ),
      BomItem(
        name: 'Anchors',
        unit: 'pcs',
        quantity: anchorCount.toDouble(),
        note: '1 anchor per leg',
      ),
    ];
  ...
  double resolveAppliedAzimuth({
    required double latitude,
    required FacingDirectionPreference preference,
  }) {
    return _resolveAppliedAzimuth(
      idealAzimuth: resolveIdealAzimuth(latitude),
  ...
  double rowSpacingFor({
    required double latitude,
    required double heightRise,
    required double interRowGapMeters,
  }) {
    return _rowSpacingMeters(
      latitude: latitude,
      heightRise: heightRise,
      interRowGapMeters: interRowGapMeters,
    );
  }
  ...
  _Layout _maximizeLayout({
    ...
    final columnPitch =
        panelSpec.spanAcrossRowMeters + panelSpec.horizontalGapMeters;
    final rowPitch = projectedRowDepth + rowSpacing;
    final maxColumns = usableWidth <= 0 || panelSpec.spanAcrossRowMeters <= 0
        ? 0
        : math.max(
            0,
            (((usableWidth + panelSpec.horizontalGapMeters) / columnPitch)
                .floor()),
          );
    ...
    for (var rows = 1; rows <= maxRows; rows++) {
      final occupiedDepth =
          (rows * projectedRowDepth) + ((rows - 1) * rowSpacing);
      if (occupiedDepth > usableDepth + 1e-9) {
        continue;
      }
      for (var columns = 1; columns <= maxColumns; columns++) {
        final occupiedWidth =
            (columns * panelSpec.spanAcrossRowMeters) +
            ((columns - 1) * panelSpec.horizontalGapMeters);
  ...
  double _resolveAppliedAzimuth({
    required double idealAzimuth,
    required FacingDirectionPreference preference,
  }) {
  ...
  double _frameSlopeLengthMeters(int rows, PanelSpec panelSpec) {
    ...
    return (rows * panelSpec.slopeRunMeters) +
        ((rows - 1) * panelSpec.verticalGapMeters);
  }

  double _frameWidthMeters(int columns, PanelSpec panelSpec) {
    if (columns <= 0) return 0.0;
    return (columns * panelSpec.spanAcrossRowMeters) +
        ((columns - 1) * panelSpec.horizontalGapMeters);
  }
  ...
  double _rowSpacingMeters({
    ...
    final shadowClearance = heightRise / math.tan(winterRadians);
    return math.max(interRowGapMeters, shadowClearance + interRowGapMeters);
  }
