...
class _SystemFormPageState extends State<SystemFormPage> {
  ...
  void initState() {
    ...
    _userPhoneCtrl = TextEditingController(text: system?.userPhone ?? (widget.isUserView ? Get.find<AuthController>().user.value?.phone : ''));
  ...
  void _save() async {
    ...
    if (widget.isUserView) {
      userPhoneToSave = Get.find<AuthController>().user.value?.phone;
    } else {
