...
import 'package:watt/src/features/admin/presentation/controllers/admin_company_details_controller.dart';
import 'package:watt/src/features/admin/presentation/forms/company_status_form.dart';
import 'package:watt/src/features/admin/presentation/widgets/admin_page_scaffold.dart';
import 'package:watt/src/features/admin/presentation/widgets/admin_section_header.dart';
import 'package:watt/src/features/admin/presentation/widgets/admin_widgets.dart';
import 'package:watt/src/features/admin/presentation/widgets/company_service_card.dart';
import 'package:watt/src/features/admin/presentation/widgets/member_list_item.dart';
...
class _AdminCompanyDetailsScreenState extends ConsumerState<AdminCompanyDetailsScreen> {
  ...
  Widget _buildHeroCard(BuildContext context, AdminCompanyDetails details) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [AppTheme.primaryColor.withValues(alpha: 0.14), Theme.of(context).cardColor]),
      ...
      child: Column(
        children: [
          ...
          Text(
            ...
            style: const TextStyle(fontFamily: AppTheme.fontFamily, fontSize: 24, fontWeight: FontWeight.w700),
          ),
  ...
  Widget _buildInfoSection(BuildContext context, AdminCompanyDetails details) {
    ...
    return _SectionCard(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const AdminSectionHeader(title: 'Company Profile', subtitle: 'Core account and business information.'),
          ...
          _InfoRow(label: 'Categories', value: company.categoryNames.isEmpty ? 'N/A' : company.categoryNames.join(', ')),
          _InfoRow(label: 'Description', value: company.description?.isNotEmpty == true ? company.description! : 'No description'),
          if (details.financials.isNotEmpty) ...[
            const SizedBox(height: 16),
            const AdminSectionHeader(title: 'Financials', subtitle: 'Aggregated values returned by the backend.'),
            ...details.financials.entries.map((entry) => _InfoRow(label: entry.key, value: '${entry.value}')),
          ],
        ],
      ),
    );
              ...
              children: details.services
                  .map(
                    (service) => Padding(
                      ...
                      child: CompanyServiceCard(service: service),
                    ),
                  )
                  .toList(),
            ),
  ...
  void _showStatusUpdateForm(BuildContext context) {
    final currentStatus = ref.read(adminCompanyDetailsProvider).details?.company.status ?? 'pending';
    ...
    showModalBottomSheet<void>(
      ...
      builder: (context) =>
          CompanyStatusForm(currentStatus: currentStatus, onSubmit: (status) => ref.read(adminCompanyDetailsProvider.notifier).updateStatus(status)),
    );
  }
}
      ...
      child: Row(
        ...
        children: [
          SizedBox(
            ...
            child: Text(
              label,
              style: TextStyle(fontFamily: AppTheme.fontFamily, fontWeight: FontWeight.w700, color: Theme.of(context).hintColor),
            ),
          ),
          Expanded(
            child: Text(value, style: const TextStyle(fontFamily: AppTheme.fontFamily, height: 1.4)),
          ),
        ],
      ),
    );
...
class _Tag extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.65), borderRadius: BorderRadius.circular(999)),
      child: Text(
        '$label: $value',
        style: const TextStyle(fontFamily: AppTheme.fontFamily, fontWeight: FontWeight.w700, color: AppTheme.primaryColor),
      ),
    );
  }
}
