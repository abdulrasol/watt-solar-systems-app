      ...
      child: Column(
        ...
        children: [
          CompanyFeatureBanner(
            ...
            subtitle: l10n.services_services_count(company.publicServices.length),
          ),
          SizedBox(height: 14.h),
          if (company.publicServices.isEmpty)
            CompanyEmptyStateCard(icon: Iconsax.briefcase, message: l10n.services_no_public_services)
          else
            ...company.publicServices.map(
              (service) => Padding(
                ...
                child: CompanyServiceShowcaseCard(
                  ...
                  priceLabel: (service.price ?? 0) > 0 ? '${priceFormat.format(service.price)} IQD' : l10n.services_price_on_request,
                ),
              ),
            ),
        ],
...
class CompanyServiceShowcaseCard extends StatelessWidget {
  ...
  const CompanyServiceShowcaseCard({super.key, required this.title, required this.description, required this.priceLabel});
      ...
      child: Column(
        ...
        children: [
          ...
          if ((description ?? '').trim().isNotEmpty) ...[
            ...
            Text(
              description!,
              style: TextStyle(fontSize: 12.sp, height: 1.55, color: Theme.of(context).textTheme.bodyMedium?.color?.withValues(alpha: 0.82)),
            ),
          ],
        ],
      ),
