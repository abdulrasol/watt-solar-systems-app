...
import 'package:watt/src/features/company_dashboard/presentation/widgets/dashboard_header.dart';
import 'package:watt/src/features/company_dashboard/presentation/widgets/overview_content.dart';
...
import 'package:watt/src/features/company_dashboard/presentation/widgets/search_results_content.dart';

class DashboardContent extends ConsumerWidget {
  final int index;
  final List<NavItem> navItems;

  const DashboardContent({super.key, required this.index, required this.navItems});
          ...
          children: [
            ...
            Expanded(
              child: searchState.isSearchActive
                  ...
                  : _buildSectionContent(context: context, currentItem: currentItem, state: state, companyId: companyId, l10n: l10n),
            ),
          ],
        ),
        if (state.isLoading) Center(child: LoadingWidget.widget(context: context)),
      ],
  ...
  Widget _buildSectionContent({
    ...
    if (currentItem.serviceCode == 'storefront_b2b' && companyId != null) {
      return StorefrontScreen(audience: StorefrontAudience.b2b, companyId: companyId, embedded: true);
    }

    if (currentItem.serviceCode == 'storefront_b2c') {
      return const StorefrontScreen(audience: StorefrontAudience.b2c, embedded: true);
    }
